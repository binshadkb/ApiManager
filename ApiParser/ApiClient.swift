...
class APIClient {
    ...
    fileprivate let baseUrl = "http://192.168.1.116/payload/"
    ...
    func get<T: Decodable>(with endpoint: String, decode: @escaping (Decodable) -> T?, params: NSDictionary?, completion: @escaping (Result<T, APIError>) -> Void) {
    ...
    func fetch<T: Decodable>(with request: URLRequest, decodingType: T.Type, completionHandler completion: @escaping (Decodable?, APIError?) -> Void) -> URLSessionDataTask {
        ...
        let postTask = session.dataTask(with: request, completionHandler: {
            ...
            if httpResponse.statusCode == 200 {
                if let data = data {
                    do {
                        let genericModel = try PayloadParser().payload(from: data, decodingType: T.self)
